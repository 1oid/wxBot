...
def Extension(request, coin):
    ...
    if coin.strip().upper() in coinKeys:
        ...
        current_price = coin_item.get("current_price")
        market_value = PriceConvert(round(float(coin_item.get("market_value")), 2))
        vol = PriceConvert(round(float(coin_item.get("vol")), 2))

        price = "当前价格: {}".format(current_price)
        percent = "涨幅: {}%".format(round(float(coin_item.get("change_percent")), 2))
        market = "流通市值: ${}".format(market_value)
        vol_24 = "24h成交: {}".format(vol)
        t = "时间: {}".format(time.strftime("%Y-%m-%d %H:%M:%S"))
        link = "https://triple.51ssd.net/m/index?channel=091NAP"
        # 返回数据给机器人
        return wxResponse(tip="\n".join([full_name, price, percent, market, vol_24, t, link]))
